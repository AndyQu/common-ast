...
public class IntegerList {
	 ...
	 * {@code true} if and only if the specified object is also an
	 * {@link IntegerList}, both lists have the same size, and all corresponding
	 * pairs of elements in the two lists are equal. In other words, two lists
	 ...
	 * an {@link IntegerList}. If not, it returns {@code false}; if so, it
	 * checks the size of both lists. If the lists are not the same size, it
	 * returns {@code false}; otherwise it iterates over both lists, comparing
